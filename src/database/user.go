...
func (o TblUser) SelectByUsername(username string) (StructureUser, *sql.DB, error) {
	...
	db, err := o.ConnectDB()
	defer db.Close()
	var query string = fmt.Sprintf(`select id, username, password, login_retry, next_login_date,email,last_login  from tbl_user WHERE username ="%s"`, username)
...
func (o TblUser) UpdateLastLogin(username string, loginTime string) (int64, *sql.DB, error) {
	db, err := o.ConnectDB()
	if err != nil {
		return 0, nil, fmt.Errorf("%s", err)
	}
	...
	var queryUpdate string
	queryUpdate = `UPDATE tbl_user set last_login = ? where username = ?`
	prepare, err := db.Prepare(queryUpdate)
